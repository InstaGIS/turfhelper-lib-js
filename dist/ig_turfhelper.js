 ...
 */function forEach(collection,iteratee){var func=isArray(collection)?arrayEach:baseEach;return func(collection,baseIteratee(iteratee,3));}/** Used to compose unicode character classes. *//**
 ...
 */function toLatLngs(coordinates){return map(coordinates,toLatLng$1);}function toCoord(LatLng){if(LatLng.lat&&LatLng.lng){return[LatLng.lng,LatLng.lat];}else if(gmaps$1&&gmaps$1.LatLng&&LatLng instanceof gmaps$1.LatLng){return[LatLng.lng(),LatLng.lat()];}else{return LatLng;}}/**
 * Transforma un array de LatLng en un array de coordenadas [lng,lat]
 ...
 */function toCoords$1(arrayLatLng,closeRing){var ring=map(arrayLatLng,toCoord);if(closeRing===true){ring.push(ring[0]);}return ring;}function latlngToPoint(latLng){var coords=toCoord(latLng),feature={type:"Feature",geometry:{type:"Point",coordinates:coords}};return feature;}/**
...
return simplifiedgeom;}else{warn('Cannot simplify  Feature',Feature);return Feature;}}function along(arrayLatLng,distance){if(arrayLatLng instanceof gmaps$1.Polyline){arrayLatLng=arrayLatLng.getPath();}var arrayCoords=toCoords$1(arrayLatLng);var LineString=turf_linestring(arrayCoords);return turf_along(LineString,distance,'kilometers');}/**
 ...
 * @return {Array}             [description]
 */function trimPaths(arrayLatLng1,arrayLatLng2,debugflag){/*if (googleGeom1 instanceof gmaps.Polyline) {
 ...
 */Wkt.Wkt.prototype.construct={/**
     ...
     * @return              {gmaps.Marker}
     */point:function point(config,component){var c=component||this.components;config=config||{optimized:true};config.position=new gmaps$1.LatLng(c[0].y,c[0].x);return new gmaps$1.Marker(config);},/**
     ...
     */linestring:function linestring(config,component){var i,c;c=component||this.components;config=config||{editable:false};config.path=[];for(i=0;i<c.length;i+=1){config.path.push(new gmaps$1.LatLng(c[i].y,c[i].x));}return new gmaps$1.Polyline(config);},/**
     ...
     */box:function box(config,component){var c=component||this.components;config=config||{};config.bounds=new gmaps$1.LatLngBounds(new gmaps$1.LatLng(c[0].y,c[0].x),new gmaps$1.LatLng(c[1].y,c[1].x));return new gmaps$1.Rectangle(config);},/**
...
verts.push(new gmaps$1.LatLng(c[j][k].y,c[j][k].x));}// eo for each vertex
...
config.paths=config.paths.concat(rings);if(this.isRectangle){return function(){var bounds,v;bounds=new gmaps$1.LatLngBounds();for(v in rings[0]){// Ought to be only 1 ring in a Rectangle
if(rings[0].hasOwnProperty(v)){bounds.extend(rings[0][v]);}}return new gmaps$1.Rectangle({bounds:bounds});}();}else{return new gmaps$1.Polygon(config);}},/**
...
if(gmaps$1.geometry){sign=gmaps$1.geometry.spherical.computeSignedArea;}// gmaps.LatLng //////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.LatLng){response={type:'point',components:[{x:obj.lng(),y:obj.lat()}]};return response;}// gmaps.Point //////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Point){response={type:'point',components:[{x:obj.x,y:obj.y}]};return response;}// gmaps.Marker //////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Marker){response={type:'point',components:[{x:obj.getPosition().lng(),y:obj.getPosition().lat()}]};return response;}// gmaps.Polyline ////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Polyline){verts=[];for(i=0;i<obj.getPath().length;i+=1){tmp=obj.getPath().getAt(i);verts.push({x:tmp.lng(),y:tmp.lat()});}response={type:'linestring',components:verts};return response;}// gmaps.Polygon /////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Polygon){rings=[];if(multiFlag===undefined){multiFlag=function(){var areas,l;l=obj.getPaths().length;if(l<=1){// Trivial; this is a single polygon
...
if(obj.constructor===gmaps$1.Circle){var point=obj.getCenter();var radius=obj.getRadius();verts=[];var d2r=Math.PI/180;// degrees to radians
...
if(obj.constructor===gmaps$1.LatLngBounds){tmp=obj;verts=[];verts.push({// NW corner
...
if(obj.constructor===gmaps$1.Rectangle){tmp=obj.getBounds();verts=[];verts.push({// NW corner
x:tmp.getSouthWest().lng(),y:tmp.getNorthEast().lat()});verts.push({// NE corner
x:tmp.getNorthEast().lng(),y:tmp.getNorthEast().lat()});verts.push({// SE corner
x:tmp.getNorthEast().lng(),y:tmp.getSouthWest().lat()});verts.push({// SW corner
x:tmp.getSouthWest().lng(),y:tmp.getSouthWest().lat()});verts.push({// NW corner (again, for closure)
...
// gmaps.Data.Feature /////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Data.Feature){return this.deconstruct.call(this,obj.getGeometry());}// gmaps.Data.Point /////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Data.Point){//console.zlog('It is a gmaps.Data.Point');
response={type:'point',components:[{x:obj.get().lng(),y:obj.get().lat()}]};return response;}// gmaps.Data.LineString /////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Data.LineString){verts=[];//console.zlog('It is a gmaps.Data.LineString');
...
if(obj.constructor===gmaps$1.Data.Polygon){rings=[];//console.zlog('It is a gmaps.Data.Polygon');
...
if(obj.constructor===gmaps$1.Data.MultiPoint){verts=[];for(i=0;i<obj.getLength();i+=1){vertex=obj.getAt(i);verts.push([{x:vertex.lng(),y:vertex.lat()}]);}response={type:'multipoint',components:verts};return response;}// gmaps.Data.MultiLineString /////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Data.MultiLineString){linestrings=[];for(i=0;i<obj.getLength();i+=1){verts=[];linestring=obj.getAt(i);for(j=0;j<linestring.getLength();j+=1){vertex=linestring.getAt(j);verts.push({x:vertex.lng(),y:vertex.lat()});}linestrings.push(verts);}response={type:'multilinestring',components:linestrings};return response;}// gmaps.Data.MultiPolygon /////////////////////////////////////////////////////
if(obj.constructor===gmaps$1.Data.MultiPolygon){polygons=[];//console.zlog('It is a gmaps.Data.MultiPolygon');
...
if(obj.constructor===gmaps$1.Data.GeometryCollection){var objects=[];for(k=0;k<obj.getLength();k+=1){// For each multipolygon
...
return'geometrycollection';}}switch(type){case gmaps$1.Marker:return'multipoint';case gmaps$1.Polyline:return'multilinestring';case gmaps$1.Polygon:return'multipolygon';default:return'geometrycollection';}}(),components:function(){// Pluck the components from each Wkt
...
return createTextMarker(options);}}};;ig_turfhelper={Wkt:Wkt,Wicket:Wicket,WKT2Object:WKT2Object,along:along,arrayToFeaturePoints:arrayToFeaturePoints,createbuffer:createbuffer,centroid:centroid,cleanFeaturePolygon:cleanFeaturePolygon,latlngToPoint:latlngToPoint,pointInPolygon:pointInPolygon,polygonToFeaturePolygon:polygonToFeaturePolygon,representGeometry:representGeometry,simplifyFeature:simplifyFeature,simplifyGeometry:simplifyGeometry,simplifyPointArray:simplifyPointArray,toCoord:toCoord,toCoords:toCoords$1,toLatLng:toLatLng$1,toLatLngs:toLatLngs,trimPaths:trimPaths,union:union,verticesInPolygon:verticesInPolygon};_export('Wkt',Wkt);_export('Wicket',Wicket);_export('WKT2Object',WKT2Object);_export('along',along);_export('arrayToFeaturePoints',arrayToFeaturePoints);_export('createbuffer',createbuffer);_export('centroid',centroid);_export('cleanFeaturePolygon',cleanFeaturePolygon);_export('latlngToPoint',latlngToPoint);_export('pointInPolygon',pointInPolygon);_export('polygonToFeaturePolygon',polygonToFeaturePolygon);_export('representGeometry',representGeometry);_export('simplifyFeature',simplifyFeature);_export('simplifyGeometry',simplifyGeometry);_export('simplifyPointArray',simplifyPointArray);_export('toCoord',toCoord);_export('toCoords',toCoords$1);_export('toLatLng',toLatLng$1);_export('toLatLngs',toLatLngs);_export('trimPaths',trimPaths);_export('union',union);_export('verticesInPolygon',verticesInPolygon);_export('default',ig_turfhelper);}};});
})
(function(factory) {
  if (typeof define == 'function' && define.amd)
