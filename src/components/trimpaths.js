...
import {traverseRings} from './traverse.js';
import {toCoords,toLatLng,toLatLngs} from './coords_to_latlng.js';
	...
	export function trimPaths(arrayLatLng1, arrayLatLng2, debugflag) {
		...
		if (kinks.intersections.features.length > 0) {
			...
			var minRing1 = _.min(kinks.intersections.features, function (kink) {
				return kink.properties.position1;
			});

			var firstIntersection = _.max(_.filter(kinks.intersections.features, function (kink) {
